...
func TestCohereProvider_EmbedMapsInputTypes(t *testing.T) {
	inputs := []struct {
		internal string
		external string
	}{
		{internal: "query", external: "search_query"},
		{internal: "document", external: "search_document"},
	}
	for _, input := range inputs {
		t.Run(input.internal, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				assert.Equal(t, input.external, body["input_type"])
			...
			got, err := p.Embed(context.Background(), []string{"text"}, input.internal)
